...
namespace Swart_Simon_17611447_Task_1
{
    class ArtilleryUnit : Unit
    {
        ...
        public ArtilleryUnit(int X, int Y, int health, int speed, int attack, string faction, string symbol, string name)
        {
            xPosition = X;
            yPosition = Y;
            Health = health;
        ...
        public void buildingCombat(Building enemy)
        {
            if (enemy.Faction != this.Faction)
            {
        ...
        public void loadRed()
        {
            ...
            inFile = new FileStream(@"Files\\redArtillery.txt", FileMode.Open, FileAccess.Read);
